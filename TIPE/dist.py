...
distance_reelle = 361
liste_distance_reelle = [86, 135, 165, 252, 303, 361]
...
sigma_x = 1
sigma_y = 1
sigma_rayleigh = np.sqrt((sigma_x**2 + sigma_y**2) / 2)
...
def generateur_coordonnees_gaussienne(liste_abcisses, liste_ordonnees, nombre_points, nombre_simulations, sigma_x, sigma_y):
    matrice_abcisses = []
    matrice_ordonnees = []
    ...
    for i in range(nombre_points):
        x0 = liste_abcisses[i]
        y0 = liste_ordonnees[i]
        
        for j in range(nombre_simulations):
            x1 = np.random.normal(x0, sigma_x)
            y1 = np.random.normal(y0, sigma_y)
            
            matrice_abcisses[j].append(x1)
            matrice_ordonnees[j].append(y1)
        
    return matrice_abcisses, matrice_ordonnees

matrice_abcisses_g, matrice_ordonnees_g = generateur_coordonnees_gaussienne(liste_abcisses, liste_ordonnees, nombre_points, nombre_simulations, sigma_x, sigma_y)

m1, m11= generateur_coordonnees_gaussienne(liste_abcisses[10], liste_ordonnees[10], 10, nombre_simulations, sigma_x, sigma_y)
m2, m22 = generateur_coordonnees_gaussienne(liste_abcisses[20], liste_ordonnees[20], 20, nombre_simulations, sigma_x, sigma_y)
m3, m33 = generateur_coordonnees_gaussienne(liste_abcisses[30], liste_ordonnees[30], 30, nombre_simulations, sigma_x, sigma_y)
m4, m44 = generateur_coordonnees_gaussienne(liste_abcisses[40], liste_ordonnees[40], 40, nombre_simulations, sigma_x, sigma_y)
m5, m55 = generateur_coordonnees_gaussienne(liste_abcisses[50], liste_ordonnees[50], 50, nombre_simulations, sigma_x, sigma_y)

def generateur_coordonnees_rayleigh(liste_abcisses, liste_ordonnees, nombre_points, nombre_simulations, sigma):
    matrice_abcisses = []
    matrice_ordonnees = []
...
matrice_abcisses_r, matrice_ordonnees_r = generateur_coordonnees_rayleigh(liste_abcisses, liste_ordonnees, nombre_points, nombre_simulations, sigma_rayleigh)
...
l = []
for i in range(5):
    l.append((distance_euclidienne(f'm{i}', matrice_ordonnees[i])))
l.appendd()
plt.plot(liste_distance_reelle, l)
plt.show()
...
print("Distance_relevee :", distance_euclidienne(liste_abcisses, liste_ordonnees))
distance_moyenne = np.mean(liste_distance)
print('Distance moyenne = :', distance_moyenne)

liste_distance_g = []
for i in range(nombre_simulations):
    liste_distance.append(distance_euclidienne(matrice_abcisses_g[i], matrice_ordonnees_g[i]))

plt.hist(liste_distance, bins='auto', alpha=0.4, edgecolor='blue')
plt.show()
print("distance reelle: ", distance_reelle)
print("Distance_relevee_g :", distance_euclidienne(liste_abcisses, liste_ordonnees))
distance_moyenne_g = np.mean(liste_distance_g)
print('Distance moyenne_g = :', distance_moyenne_g)

liste_distance_r = []
for i in range(nombre_simulations):
    liste_distance_r.append(distance_euclidienne(matrice_abcisses_r[i], matrice_ordonnees_r[i]))

plt.hist(liste_distance, bins='auto', alpha=0.4, edgecolor='blue')
plt.show()
print("distance reelle: ", distance_reelle)
print("Distance_relevee_r :", distance_euclidienne(liste_abcisses, liste_ordonnees))
distance_moyenne_r = np.mean(liste_distance_r)
print('Distance moyenne_r = :', distance_moyenne_r)
...
matrice_segments_g = matrice_segment(matrice_abcisses_g, matrice_ordonnees_g)
matrice_segments_r = matrice_segment(matrice_abcisses_r, matrice_ordonnees_r)
...
matrice_covariance_u = matrice_covariance(liste_segments, matrice_segments)
matrice_covariance_g = matrice_covariance(liste_segments, matrice_segments_g)
matrice_covariance_r = matrice_covariance(liste_segments, matrice_segments_r)
'''
# Visualiser la matrice de covariance
plt.figure(figsize=(10, 8))
sns.heatmap(matrice_covariance, annot=True, cmap='coolwarm', linewidths=0.5)
...
matrice_correlation_1 = coefficient_correlation(matrice_covariance_u)
matrice_correlation_g = coefficient_correlation(matrice_covariance_g)
matrice_correlation_r = coefficient_correlation(matrice_covariance_r)
...
incertitude_u = calcul_incertitude(matrice_correlation_1, matrice_covariance_u)
incertitude_g = calcul_incertitude(matrice_correlation_g, matrice_covariance_g)
incertitude_r= calcul_incertitude(matrice_correlation_r, matrice_covariance_r)
print("Incertitude: ", incertitude_u)
print("Incertitude:_g ", incertitude_g)
print("Incertitude_r: ", incertitude_r)

def z_score(x_reel, x_moyenne, incertitude):
    return (x_reel - x_moyenne)/np.sqrt(incertitude)

print("z_score: ", z_score(distance_reelle, distance_moyenne, incertitude_u))
print("z_score: ", z_score(distance_reelle, distance_moyenne_g, incertitude_g))
print("z_score: ", z_score(distance_reelle, distance_moyenne_r, incertitude_r))
